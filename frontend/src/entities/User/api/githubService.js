...
// Helper to get headers with our shiny new token!
const getHeaders = () => ({
...
export const fetchProfile = async (username) => {
  const res = await fetch(`${BASE_URL}/users/${username}`, { headers: getHeaders() });
...
export const fetchRepos = async (username) => {
  ...
  const res = await fetch(`${BASE_URL}/users/${username}/repos?per_page=100&sort=pushed`, { headers: getHeaders() });
